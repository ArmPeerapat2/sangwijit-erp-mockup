...
When a built mockup's module code or scope disagrees with the `Flow Design/` flowcharts and the `sangwijit-portal-skill/modules/` spec, the Flow + spec win and the mockup is corrected to match — not the other way around.
