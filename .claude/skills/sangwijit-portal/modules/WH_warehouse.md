...
Trigger: ของถึงคลัง (อ้างใบสั่งซื้อ) หรือ Transfer Order ส่งออกแล้ว
Output:  Serial ลงใน BC + จัดวาง Bin Location + สต็อกเพิ่ม
```

### ความสัมพันธ์กับฝ่ายจัดซื้อ/บัญชี — ⛔ ไม่มีหน้ารับของฝั่งบัญชีแล้ว

> **ใบรับสินค้าของคลัง = เจ้าเดียวที่รับของเข้าสต็อก** (ตัดหน้ารับฝั่งบัญชี 2026-06-08 · ยืนยันซ้ำ 2026-07-21)
> - ฝ่ายบัญชี/จัดซื้อ **ไม่รับของเอง** — เข้าดูใบรับสินค้าของคลังผ่าน**สิทธิ์ RBAC**
> - **ใบตั้งหนี้เจ้าหนี้** อ่านยอดรับสะสมจากใบรับสินค้าของคลังเป็น gate — ไม่สร้างการรับซ้ำ
> - **ใบตั้งหนี้/ใบวางบิล/ใบสั่งซื้อ ห้ามสร้างแถวเคลื่อนไหวสต็อก** (กันนับซ้ำ)

```
Purchase Receipt Flow (ปัจจุบัน):
  ใบสั่งซื้อ (จัดซื้อ): สั่งของ — ไม่แตะสต็อก
      ↓ ของถึงคลัง
  ใบรับสินค้า WH-1 (คลัง): รับเข้าสต็อก + ลง Serial + จัดวาง Bin  ← จุดเดียวที่สต็อกเพิ่ม
      ↓ อ่านยอดรับสะสมเป็น gate
  ใบตั้งหนี้เจ้าหนี้ (บัญชี): ตั้งหนี้ + จับคู่ 3 ทาง (อ่านอย่างเดียว ไม่แตะสต็อก)
