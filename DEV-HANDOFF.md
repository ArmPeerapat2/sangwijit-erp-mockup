...
## 🔒 Single-Source & Inherit-Lock — งานขาย (SL) + จัดซื้อ (PO) (audit + Peerapat เคาะ 2026-06-26 → 07-20)

> กติกาห้าม dev ละเมิด: คำนวณที่เดียว · สืบทอดโดยอ้างอิง (ไม่ copy ค่า) · หน้าปลายทาง = mirror อ่านอย่างเดียว

### 💰 งานขาย (SL)

**1) Calc engine เดียว — `swtCalc`/`swtRenderSummary` (`swt-patterns.js`)**
- ทุกเอกสาร (ใบเสนอราคา · ใบจอง · ใบมัดจำ · บิลขาย · ใบลดหนี้) เรียก function เดียวกัน · **ห้ามเขียนสูตรซ้ำต่อหน้า**
- VAT = Base×7% ตาม Golden Rule (ส่วนลดหักก่อน VAT) · ผล engine ต้องตรงกับ test fixture เสมอ

**2) Inherit-by-reference ตลอดลูกโซ่** ใบเสนอราคา → ใบจอง → บิลขาย → ใบลดหนี้

| ฟิลด์ | กติกา (เคาะแล้ว) |
|---|---|
| ลูกค้า | **lock** จากใบต้นทาง — แก้ไม่ได้ทั้งลูกโซ่ |
| ราคาต่อหน่วยในบิลขาย | **lock** ตามใบจอง — แก้ไม่ได้ |
| จำนวนออกบิล | ≤ ที่จอง (partial ได้ · ลดได้ ไม่เพิ่ม) |
| เพิ่มรายการใหม่ | ได้เฉพาะชั้น **ใบจอง** · ใช้ราคา/โปรปัจจุบัน (ไม่ inherit ราคาเก่า) · ห้ามต่ำกว่าทุน |
| ส่วนลดท้ายบิล | เพิ่มได้ตามสิทธิ์ — ผูก CF-2.6 Approval Matrix |
| ยอดรวม / VAT | **derived** จาก engine เท่านั้น — ห้าม override |

**3) SoT ต่อ master (ปลายทาง = read-only mirror):** Floor/MAP → PM-1 รายการราคา · Rebate → PO-7 · AR Aging → AR · Tier ส่วนลด → CF-2.6

**4) ห้าม fork:** ใบลดหนี้จากเคลมใช้ engine SL-CN ตัวเดียว (เส้นทาง SV-4→SL-Q→SL-CN) · UI components (badge/KPI/filter/queue/detail-panel) = shared เท่านั้น

### 📦 จัดซื้อ (PO)

**1) 3-Way Match = engine จุดเดียว — ใบตั้งหนี้เจ้าหนี้ (PO-6) เป็นเจ้าของ compute**
- หน้าอื่น (คิวจัดซื้อ · บิลฝาก) แสดงผล **read-only** เท่านั้น
- **Gate 2 จุด:** (a) **รับของ**ที่อ้างใบสั่งซื้อ → รับเกิน/รับผิด **block ตอนรับ** · รับไม่อ้างใบสั่งซื้อได้ตามสิทธิ์ (b) **ตั้งหนี้** → 3-way ครบ + ดึง**มัดจำจ่ายที่อ้างใบสั่งซื้อ**มาหักอัตโนมัติ (จ่ายก่อนเพื่อส่วนลดเงินสด = อ้างใบสั่งซื้อ ไม่ใช่อ้างบิล)
- "รับครบ" = รับสะสม = สั่ง **ทุกไลน์ qty-based** (locked 2026-06-07) · บิลฝาก = 2-way (PO-6 mode toggle มีแล้ว)

**2) ราคาใบสั่งซื้อ (เคาะ 2026-07-20):**
- อ้างใบขอซื้อ/ใบเสนอราคา vendor = **lock** · เปิดเองไม่อ้างเอกสาร = โชว์ราคาล่าสุดที่เคยได้เป็น hint แก้ได้
- **ราคาใบวางบิล vendor ≠ ใบสั่งซื้อ → ยึดราคาใบสั่งซื้อเสมอ** — ตั้งหนี้ตามราคาที่ตกลง · ส่วนต่าง = เจรจาให้ vendor แก้บิล/ออกใบลดหนี้ (PO-CN) · vendor ขึ้นราคาจริง = แก้ที่ใบสั่งซื้อ (audit trail) ไม่ยอมตามบิล

**3) Inherit ตาม chain (อ้างอิง ไม่ copy):**

| ฟิลด์ | SoT |
|---|---|
| เครดิตเทอม / due date | ทะเบียนเจ้าหนี้ (MD-3) — due date compute อัตโนมัติ |
| GRN qty / สถานะรับ | คลัง (WH-1 = SoT) · ฝั่งจัดซื้อบริโภค read-only |
| ข้อตกลงส่วนลด/โควต้า | PO-2 · PO-7 อ่านอย่างเดียว (po7PullPo2) |
| ใบตั้งหนี้ | inherit เต็มจากใบวางบิล vendor (เลข invoice · GRN lines · ไฟล์แนบ = document id เดียว) — บันทึกที่ PO-6 · ไม่มีหน้า VBL แยก |

**4) กัน double-count:** บิลฝาก dispatch อ้าง INV/TRF ของ Sales/WH แบบ **read-only** — ห้ามสร้าง record/ยอดซ้ำ
