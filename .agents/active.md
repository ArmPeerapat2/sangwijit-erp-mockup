...
- **🔓 REOPENED สิทธิ์พนักงาน (2026-07-24) — Peerapat ชี้ระบบเก่า Account Hero:** "อยากได้แนวคิดเดียวกับตัวเดิมเลย" → ถอดโครงสิทธิ์เก่าครบที่ `.agents/topics/legacy-permission-model.md` (จาก 7 ภาพ `Hero Exam\User`). **โมเดลจริง = 3 ระดับ** (พนักงาน 9.1 / กลุ่มสิทธิ์ 9.5 / เอกสาร CSSALE) ผูกที่ **"กลุ่มสิทธิ์การใช้งาน"** + override รายคน + ขอบเขตสาขา/แผนก + สิทธิ์มีอายุ. **🔴 มติ 07-24 "ไม่มี override รายคน" = กลับ** — ระบบเก่ามี 27 ช่องรายคนชัดเจน. **3 ตัวแยกกัน:** ตำแหน่ง ≠ กลุ่มพนักงาน ≠ กลุ่มสิทธิ์ (portal ยุบผิด). แผน build 3 เฟส (reconcile → ชั้นเจ็บ → ชั้นลึก) ดู reference file
