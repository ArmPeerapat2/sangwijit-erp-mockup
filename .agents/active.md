...
- **💰 ระบบราคา — grill เคาะโมเดล + เริ่ม build (Aราคา เสร็จ · B/C ต่อพรุ่งนี้):**
  - **grill locked:** ราคามาตรฐาน=default ตามกลุ่มลูกค้า · **โปรฯ(ช่วงเวลา)แสดงก่อน** (priority: โปรฯ→กลุ่ม) · **ล็อกราคาตอนทำใบจอง** (อ้างราคาเก่าได้) · ประวัติราคา=เอาครบ 3 (snapshot ตอนขาย·timeline ต่อสินค้า·สถานะล่าสุด) · **ทุกการเปลี่ยนราคาต้องขออนุมัติ** (ใบขอเปลี่ยน CSSTDPRICEREQ→CF-2.6)
  - **✅ Aราคา เสร็จ (pm1):** เพิ่ม note ลำดับราคา (priority) · **PM-1.3 ประวัติราคา (timeline ต่อสินค้า จริง)** — เก็บทุกเวอร์ชัน SUB (v1/v2/v3 · ราคาเก่า→ใหม่ ·%· ใบขอเปลี่ยน PM-3 · ผู้อนุมัติ · ✅ ล่าสุด/⚪ เก่า) · สลับสินค้าได้ · แก้ปัญหาระบบเดิมไม่เก็บประวัติ
  - **✅ Aราคา+ (07-29): PM-1.2 ตั้งราคาแบบกลุ่ม** — grill เพิ่ม: ตั้งราคาทีละสินค้า/รุ่น **แต่ค้นเป็นกลุ่มได้** (ค้นหา·หมวด·ยี่ห้อ) → เลือกทีละ/หลายรายการ (checkbox) → **ปรับราคาชุด** (ตั้งเป็น/+−%/+−บาท) · ปรับแล้ว=ไฮไลต์รอ **ส่งขออนุมัติ** (ผูกกฎ ทุกการเปลี่ยนต้องอนุมัติ) · verify: กรอง+bulk +10% ถูก
  - **⏳ ทำต่อ:** **โปรฯ multi-model** (grill 07-29: โปรโมชั่นใส่พร้อมกันหลายรุ่นได้) → หน้าโปรฯ PM-2 · **Bราคา** = หน้าใหม่ `pm3-price-change-request` (ใบขอเปลี่ยนราคา + อนุมัติ CF-2.6 → ลง timeline) · **Cราคา** = ขายทันราคา SL-1/2/4 (resolve โปรฯ→กลุ่ม + snapshot + ล็อกที่ใบจอง)
...
- **🔒 เคาะ (2026-07-28): SC-3P รับชำระเงิน = จอรับเงินมาตรฐานทุกจุดรับเงิน** (มัดจำ · บิลขาย · บริการ · รับชำระ AR) — Peerapat: "ไม่ว่าจะรับมัดจำ บิลขาย service หรือรับชำระต่างๆ ให้เรียก SC-3P ขึ้นมาใช้ได้เลย" · สถานะ wiring: FI-1 (ปุ่มบันทึก 67f342a) · SL-3 (เพิ่มงวดมัดจำ · มีอยู่แล้ว) · SV-7 (แท็บรับชำระ inline · มีอยู่แล้ว) · **SL-4 เพิ่มใหม่** — ปุ่ม "💳 รับชำระเงิน (SC-3P)" ใน saab-more (เก็บหน้าร้าน/ปลายทางจากบิลได้เลย · แยกจากปุ่มไป FI-1 ฝั่งบัญชี) · sqt/sv4/fiq/sv1 ไม่ใช่จุดรับเงินตรง (ถูกแล้ว) · demo กลาง = การ์ด SC-3P ใน sc-shared-catalog (หน้า demo เดี่ยว archive ไปแล้ว 07-21 · ไม่กู้)
- **🔒 SC-3P รับชำระเงิน = จอ amount-first (swt-panels.js) เป็นตัวจริงตัวเดียว (2026-07-28 · ae71ce6):** เดิม swtRenderPayment ชนกัน 2 ไฟล์ (panels amount-first vs payment.js modal) · เคาะใช้ amount-first (แบบในแคตตาล็อก · มีวงเงินเครดิต/approval) · เพิ่ม swtOpenPayment เข้า panels (modal wrapper) · retire swt-payment.js (ตัดออก fi1/sl3/sl4/sv7 · ไม่มีใครโหลดอีกนอก _archive) · ทุกจุดรับเงินเรียก swtOpenPayment เด้งจอเดียวกัน
- **✅ SV multi-axis เต็มตัว — spec + sv-q (2026-07-28):** ทำตาม Peerapat "ทำเต็มตัว" · (1) spec §13.5 โมเดล 7 แกน (source of truth · 9-stage=engine · 7แกน=มุมมอง) · (2) flow-service-status.html diagram (91e7507) · (3) **sv-q คิว: แต่ละ job โชว์ badge หลายแกน** (data-ax ต่อ row · JS render svax-stack · 12/14 job หลายแกนจริง เช่น SV-0148 = 🔧กำลังซ่อม·💰รับมัดจำ·🔩รอรับ) + **filter ต่อแกน** (รับเงิน/อะไหล่/ซากคืน/ค่าแรง/เคลม/เบิกประกัน) · browser-verified · console clean · **เหลือ rollout: sv1/sv4/sv5 status track ต่อแกน (ทีหลัง)**
