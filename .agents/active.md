...
- **✅ wire สิทธิ์ field-level ทำงานจริง (demo · 2026-07-27):** สร้าง `swt-perm.js` (permission store กลาง + `swtApplyPerm`/`swtSetRole` + แถบ "🧪 จำลองสิทธิ์" มุมจอ · preset role ผจก./พนักงานขาย/แคชเชียร์) ต่อยอด pattern sc2 view-cost · โหมด **lock** (เทา+disabled) / **hide** (ย้ายค่าไป JS memory ล้าง DOM · ตาม sc2 ไม่ให้ DevTools เห็น) · wire **sl4** (ราคา/ส่วนลดรายบรรทัด tag ตามคอลัมน์ + ปุ่มเปลี่ยนลูกค้า + แถบวงเงิน hide) + **po6** (ปุ่มเปลี่ยนเจ้าหนี้ + AP คงค้าง hide) · **หมายเหตุ: CF-1 สิทธิ์ build ครบแล้ว** (Phase 1+2+3 ส่วนใหญ่ · ผมเคยสรุปผิดว่ายังไม่ build — เชื่อ note เก่าไม่เปิดไฟล์) · po6 เป็นหน้าจับคู่บิล ไม่มี line ราคา → demo line เต็มควรใช้ po4 (PO) · ⚠️ browser pane ค้าง verify สดไม่ได้ · code: node --check ผ่าน · wiring ครบถูกที่
