...
- **📌 backlog CSS rebuild (list เก่า inline CSS ไม่ใช้ shared):** fi3 · fi4 · wh2 · wh4 · wh-nm (+ wh-r link แต่ไม่ใช้) — migrate เข้า swt-patterns.css ทีหลังทั้งชุด (งาน refactor ไม่ใช่ patch)
...
- **📌 backlog: shared swt-sc-wire.js `migrateSummary` clobber** — เจอตอน slcn: migrateSummary เขียนทับการ์ดสรุปที่ `.row-bottom > .card:last-child` ด้วย placeholder ผิด (subtotal 12,000) · เลี่ยงได้โดยย้ายสรุปออกจาก row-bottom · ควรเช็ค/แก้ shared
